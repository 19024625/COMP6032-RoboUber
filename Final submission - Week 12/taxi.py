...
class Taxi:
      ...
      def _DFS(self, start, target, ply=0, explored = None):
            if start == target:
               return [start]
            if ply[0] <= 0:
               return []   
            if explored is None:
               explored = set([start])   
            bottomOut = False         
            for nextNode in (node for node in self._map[start] if node not in explored):
               pathExists = DFS(nextNode, target, ply-1, explored.union(set([nextNode])))
               if pathExists is not None:
                  if len(pathExists) > 0:
                     pathExists.insert(0,start)
                     return pathExists
                  else:
                     bottomOut = True
            if bottomOut:
      ...
      def _planPath(self, origin, destination, heuristic = None, **args ):
         ...
         def DFS(self,origin,destination, **args):
                

            #An attempt at probabilistic path planning.
            ...
            #    explored = set([origin])
            reachEnd = False
            for nextNode in (node for node in self._map[origin].keys() if node not in args['explored']):
               path = DFS(nextNode, destination, ply - 1, args['explored'].union(set([nextNode])))
               if path is not None:
                  if len(path) > 0:
                     path.insert(0,origin)
                     print("path from DFS", path)
                     return path
                  else:
                     reachEnd = True
               if reachEnd:
         ...
         ply = 1
         optimalPath = []
         while optimalPath is not None and len(optimalPath) == 0:
            optimalPath = self._DFS(self, origin, destination, ply)
            ply += 1
         return optimalPath
         ...
         # exploredNodes = set()
         # expandedNodes = {heuristic(origin, destination): {origin: [origin]}}
         # while len(expandedNodes) > 0:
         #    optimalPath = min(expandedNodes.keys())
         #    nextExpansion = expandedNodes[optimalPath]
         #    if origin in nextExpansion:
         #       return nextExpansion[origin]
         #    nextNode = nextExpansion.popitem()
         #    while len(nextExpansion) > 0 and nextNode[0] in exploredNodes:
         #       nextNode = nextExpansion.popitem()
         #    if len(nextExpansion) == 0:
         #       del expandedNodes[optimalPath]
         #    if nextNode[0] not in exploredNodes:
         #       exploredNodes.add(nextNode[0])
         #       expandedDestination = [node for node in self._map[nextNode[0]].items() if node[0] not in explored]
         #       while len(expandedDestination) > 0:
         #          distance = optimalPath - heuristic(nextNode[0], destination) + (expandedDestination.pop())[1] + heuristic((expandedDestination.pop())[0], destination)
         #          if distance in expandedNodes:
         #             expandedNodes[distance][(expandedDestination.pop())[0]] = nextNode[1] + [(expandedDestination.pop())[0]]
         #          else:
         #             expandedNodes[distance] = {(expandedDestination.pop())[0]: nextNode[1] + [(expandedDestination.pop())[0]]}
         ...
         # MY IMPLEMENTATION IS NOT BEING USED, SINCE I CAN NOT GET IT WORKING CORRECTLY, SO IT IS COMMENTED SO I CAN TEST OTHER PARTS OF THE CODE
         # I have multiple implementations, to see what I could get working

         # #Breadth first implementation
         # if 'explored' is not one of the arguments that is passed when calling the method, a blank dictionary will be made
         # if 'explored' not in args:
         #    args['explored'] = {}
         # #Create a empty set to put nodes that have been explored during traversal
         # explored = set()
         # #Create empty dictionary to store what path can be taken to go to a specific node
         # path = {}
         # #frontier is a list of all possible nodes which are not explored, in the map
         # frontier = [node for node in self._map[origin].keys() if node not in args['explored']]
         
         # #Checks that frontier still has nodes in
         # while len(frontier) > 0:
         #    #Retrive closest node from top of dictionary

         #    currentNode = frontier.pop(0)
         #    explored.add(currentNode)
         #    #expansion is a dictionary with every node which is connected to where the taxi is
         #    expansion = dict([(node,currentNode) for node in self._map[currentNode].keys()
         #                     if node not in explored and node not in path])
         #    #if the starting point of the taxi is in the expansion nodes, 
         #    if origin in expansion:
         #       #Create a list which will be the path taken, which at the moment just has the destination (end point)
         #       foundPath = [destination]
         #       #Parent is the top part of the branch of the tree currently being traversed
         #       parent = currentNode
         ...
         #          parent = path[parent]
         #       foundPath.append(parent)
         #       #reverse the dictionary, so it goes from origin, to destination
         #       foundPath.reverse()
         #       #Returns foundPath so that it can be used in RoboUber.py
         #       return foundPath
         #    else:
         #       frontier.extend(expansion.keys())
         ...
         #Implementation of iterative deepening search, to make path planning more efficient
         ...
         # # My implementation of path planning
         # if 'explored' not in args:
         #    args['explored'] = {}
         # args['explored'][origin] = None 
         # if origin == destination:
         #    return [origin]
         # self._onDutyPos = [origin]

         # explored = set()
         # path = {}
         # while len(self._onDutyPos) > 0:
         #    now = self._onDutyPos.pop(0)
         #    explored.add(now)
         #    expansion = dict([(node,now) for node in self._map[now].keys()
         #                      if node not in explored and node not in path])
         #    if origin in self._map:
         #       optimalPath = [destination]
         #       parent = now
         #       while parent in path:
         ...
         #     now = self._frontier.pop(0)
         #     explored.add(now)
         #     unknown = dict(node, now)
         #  knownPath = []
      ...
      def _bidOnFare(self, time, origin, destination, price):
         ...
         #==========My Implementation==========
          ...
          #I attempted at creating the variables, domains and costraints needed for the csp solver, but was not successful.
          cspVars = [CSP.CSPNode('start',((0,origin[0],destination[1]),))]
          dom = {}
          constraints = []
          ...
          domain = {NoCurrentPassengers, NoAllocatedFares, TimeToOrigin,TimeToOrigin,FiniteTimeToOrigin,FiniteTimeToDestination}
